...
public class Administration {
	...
	public int c=0;
	public int UserId=0;
	public int[] UserNumberOfTickOfService = {815,
			704,
			1056,
			1081,
			1052,
	...
	public int FindNextUser(){
		...
		for(User user:waitingQueue){
			if(((RunEnvironment.getInstance().getCurrentSchedule().getTickCount()-waitingQueue.get(i).arrivalTick)/waitingQueue.get(i).numberOfTickOfService)>ratio){
				isRatio=true;
				ratio=((RunEnvironment.getInstance().getCurrentSchedule().getTickCount()-waitingQueue.get(i).arrivalTick)/waitingQueue.get(i).numberOfTickOfService);
				min=i;
			}else{
				if(waitingQueue.get(min).numberOfTickOfService > waitingQueue.get(i).numberOfTickOfService && isRatio==false){
					min=i;
				}
			}
			i++;
	...
	public int FindNextUser(){
		...
		for(User user:waitingQueue){
			if(time>inter){
				...
				if(waitingQueue.get(min).numberOfTickOfService > waitingQueue.get(i).numberOfTickOfService){
					min=i;
					time-=waitingQueue.get(i).numberOfTickOfService;
				}
			}else{
				if(waitingQueue.get(min).numberOfTickOfService > waitingQueue.get(i).numberOfTickOfService && isTooLong==false){
				min=i;
				time+=waitingQueue.get(i).numberOfTickOfService;
				}
			}
	...
	public void addUser() {
		...
		if (AdministrationBuilder.commingTicks.contains(RunEnvironment.getInstance().getCurrentSchedule().getTickCount())){
	    	...
	    	User user = new User(space, grid, UserNumberOfTickOfService[UserId], RunEnvironment.getInstance().getCurrentSchedule().getTickCount());
	    	UserId+=1;
